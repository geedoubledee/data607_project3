...
## Filtering For Historical Important Bigrams
...
## Filtering For Historical Important Trigrams
...
historical_important_trigrams$topic <- replicate(nrow(historical_important_trigrams), '')
...
## Comparing Important Single Words and Historical Important Single Words

```{r}
important_single_words$diff_in_norm_sum <- important_single_words$tf_dc_norm_sum-historical_important_single_words$tf_dc_norm_sum
important_single_words

plot_single_words <- important_single_words %>%
    arrange(desc(diff_in_norm_sum)) %>%
    top_n(10)
```


## Bar Chart on Single Words

```{r}
ggplot(plot_single_words, aes(x=reorder(`word`, -tf_dc_norm_sum), y=tf_dc_norm_sum)) +
    geom_bar(stat="identity", position="dodge", width=0.7) +
    theme_minimal() +
    labs(title="Difference from Current and Historical Important Single Words",
       y="Norm Sum Difference",
       x="Single Words") +
    coord_flip()
```


## Summary on Single Words
```{r}
summary(comparedf(historical_important_single_words, important_single_words, by="doc_count"))
```


## Comparing Important Bigrams and Historical Important Bigrams

```{r}
important_bigrams$diff_in_norm_sum <- important_bigrams$tf_dc_norm_sum-historical_important_bigrams$tf_dc_norm_sum

plot_bigrams <- important_bigrams %>%
    arrange(desc(diff_in_norm_sum)) %>%
    top_n(10)
```
...
## Bigrams Chart
```{r}
ggplot(plot_bigrams, aes(x=reorder(`bigram`, -tf_dc_norm_sum), y=tf_dc_norm_sum)) +
    geom_bar(stat="identity", position="dodge", ) +
    theme_minimal() +
    labs(title="Difference from Current and Historical Important Bigrams",
       y="Norm Sum Difference",
       x="Bigrams") +
    coord_flip()
```
...
## Summary on Bigrams
```{r}
summary(comparedf(historical_important_bigrams, important_bigrams, by='bigram'))
```

## Comparing Important Trigrams and Historical Important Trigrams

```{r}
# Subtract
important_trigrams$diff_in_norm_sum <- important_trigrams$tf_dc_norm_sum-historical_important_trigrams$tf_dc_norm_sum

plot_trigrams <- important_trigrams %>%
    arrange(desc(diff_in_norm_sum)) %>%
    top_n(10)
```
...
## Charts on Trigram 
```{r}
ggplot(plot_trigrams, aes(x=reorder(`trigram`, -tf_dc_norm_sum), y=tf_dc_norm_sum)) +
    geom_bar(stat="identity", position="dodge") +
    theme_minimal() +
    labs(title="Difference from Current and Historical Important Trigrams",
       y="Norm Sum Difference",
       x="Trigrams") +
    coord_flip()
```
...
## Summary on Trigrams
```{r}
summary(comparedf(historical_important_trigrams, important_trigrams,by='trigram'))
```

## Combine Bigrams both current and historical dataset
```{r}
#important bigram comparsion
bigram_comparsion <- merge(subset(important_bigrams, select = c("bigram", "topic", "diff_in_norm_sum")), subset(historical_important_bigrams, select = c("bigram", "topic")), by = c("bigram", "topic"))

colnames(bigram_comparsion) <- c("multi-gram words", "topic", "norm_sum_difference")

bigram_comparsion$norm_sum_difference <- round(bigram_comparsion$norm_sum_difference, 3)

head(bigram_comparsion, 100)

```

## Combine Trigram historical and current dataset
```{r}
head(important_trigrams)
head(historical_important_trigrams)
trigram_comparsion <- merge(subset(important_trigrams, select = c("trigram", "topic", "diff_in_norm_sum")), subset(historical_important_trigrams, select = c("trigram", "topic")), by = c("trigram", "topic"))

colnames(trigram_comparsion) <- c("multi-gram words", "topic", "norm_sum_difference")

trigram_comparsion$norm_sum_difference <- round(trigram_comparsion$norm_sum_difference, 3)

head(trigram_comparsion, 100)
```

```{r}
#combine both bigram and trigram difference into one dataframe.
multi_gram_data <- rbind(bigram_comparsion, trigram_comparsion) %>%
  arrange(desc(norm_sum_difference))

#top 10 and bottom 10
top_10 <- multi_gram_data %>%
  top_n(10)

bottom_10 <- multi_gram_data %>%
  top_n(-10)

big_difference_data <- rbind(top_10, bottom_10) %>%
  arrange(desc(norm_sum_difference))

#diverging chart
color <- ifelse(big_difference_data$norm_sum_difference < 0, "pink", "lightgreen")
...
ggplot(big_difference_data, aes(x = reorder(`multi-gram words`, norm_sum_difference), y = norm_sum_difference)) +
  ...
  geom_text(aes(label = norm_sum_difference,
                hjust = ifelse(norm_sum_difference < 0, 1.5, -1),
                vjust = 0.5),
            size = 3) +
  xlab("Word") +
  ylab("Difference") +
