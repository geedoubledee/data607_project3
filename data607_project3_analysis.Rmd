...
```{r}
#
single_words_comparsion <- merge(subset(important_single_words, select = c("word", "tf_dc_score")), subset(historical_important_single_words, select = c("word", "tf_dc_score")), by = "word")

colnames(single_words_comparsion) <- c("word", "current_tf_dc_score", "historical_tf_dc_score")

# important single words
single_words_10 <- single_words_comparsion %>%
  arrange(desc(current_tf_dc_score)) %>%
  slice(1:10)

# lolipop chart
ggplot(data = single_words_10) +
  geom_segment(aes(x = reorder(word, current_tf_dc_score), xend = reorder(word, current_tf_dc_score), y = current_tf_dc_score, yend = `historical_tf_dc_score`), color = "gray", lwd = 1) +
  geom_point(aes(x=word, y = current_tf_dc_score), color=rgb(0.2,0.7,0.1,0.5), size=5) +
  geom_point( aes(x= word, y = historical_tf_dc_score), color=rgb(0.7,0.2,0.1,0.5), size=5) +
  geom_text(aes(x=word, y = current_tf_dc_score, label = current_tf_dc_score), color = "black", size = 1.5) +
  geom_text(aes(x=word, y = historical_tf_dc_score, label = historical_tf_dc_score), color = "black", size = 1.5) +
...
```{r}
# important bigram chart
ggplot(important_bigrams, aes(fill = bigram, y = tf_dc_score, x = topics)) +
  geom_bar(position="dodge", stat="identity")
  #ggtitle("Important Bigrams Word") +
  #facet_wrap(~important_bigrams)
  #theme_ipsum() +
