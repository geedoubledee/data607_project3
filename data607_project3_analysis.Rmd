...
single_topics_10 <- important_single_words %>%
  arrange(desc(tf_dc_score)) %>%
  slice(1:10)

# lolipop chart
ggplot(data = single_words_10, aes(x= `word`, y = `tf_dc_score`)) +
  geom_segment(aes(x = reorder(`word`, `tf_dc_score`), xend = reorder(`word`, `tf_dc_score`), y = 0, yend = `tf_dc_score`),
               color = "gray", lwd = 1) +
  geom_point(size = 7.5, pch = 21, bg = 4, col = 1) +
  geom_text(aes(label = `tf_dc_score`), color = "black", size = 2.25) +
