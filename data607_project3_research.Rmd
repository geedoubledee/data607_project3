...
theoretical <- c('statistics', 'mathematics', 'algorithums', 'ai', 'optimization', 'research', 'modeling', 'models', 'ml')

soft_skills <- c('collaborate', 'leadership', 'product')

data_storage <- c('cloud', 'aws', 'datasets', 'database', 'oracle', 'sql')

business_interest <- c('marketing', 'product')

topics <- c(theoretical = 'theoretical', soft_skills = 'communication / people skills', programming = 'programming', data_storage = 'data management', presentation = 'presentation', business_interest = 'business interests')
...
for (x in 1:length(topics)) {
  for (y in 1:nrow(important_single_words))
  important_single_words$topic[important_single_words$word[y] %in% topics[x]] = topics[x]
}

copy <- as.data.frame(important_single_words$word)
cbind(important_single_words,copy)
x <- important_single_words %>%
  filter(word %in% theoretical) %>%
  important_single_words$copy <- 'theoretical'


important_single_words

important_single_words$topic[important_single_words$word %in% languages] = 'programming'
important_single_words$topic[important_single_words$word %in% data_storage] = 'data management'
important_single_words$topic[important_single_words$word %in% theoretical] = 'theory'
important_single_words$topic[important_single_words$word %in% presentation] = 'reporting'
important_single_words$topic[important_single_words$word %in% soft_skills] = 'communication / people skills'
```
