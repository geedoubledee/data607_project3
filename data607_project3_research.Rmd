...
```{r}
text_df_clean$Text <- str_replace_all(text_df_clean$Text, "[^[:alnum:]]", " ")
```

```{r}
text_df_clean$Text <- iconv(text_df_clean$Text, from='', to="ASCII//TRANSLIT")
```



```{r}
#text_df_clean$Text <- strsplit(gsub("[^[:alnum:] ]", "", text_df_clean), " +")[[1]]
```



```{r}
#text_df_clean$Text <- str_split(str_squish((str_replace_all(text_df_clean$Text, regex("\\W+"), " "))), " ")
```
...
```{r}
#text_df
#text_df2 <- iconv(text_df$Text, from = 'UTF-8', to = 'ASCII//TRANSLIT') #removes 
#text_df1 <- str_replace_all(text_df$Text, "[[:punct:]]", " ")
#str_replace_all(text_df$Text, "[^[:alnum:]]"," ") #removes non-alphanumeric
```
